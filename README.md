...
1. The self-marking is issued through a self mark / macro command (the same you would use for in game macro) and is only issued when you press a button and only done so for your spread (exactly the same as an ingame macro) this does not read combat data to make that determination or perform an action.
